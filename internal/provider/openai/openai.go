...
func New(cfg provider.Config) (provider.Provider, error) {
	...
	mimo := protocol == "" && IsMiMo(cfg.BaseURL)
	switch {
	case protocol == "none":
		effort = ""
	case deepseek:
	...
	case mimo:
		// MiMo uses the standard reasoning_effort scale (low/medium/high)
		// but also supports reasoning_content in the response.
		// "max" is not a valid value for MiMo, so clamp it to the ceiling.
		switch effort {
		case "max":
			effort = "high"
...
type client struct {
	...
	mimo         bool          // true for xiaomimimo.com — supports reasoning_content and reasoning_effort
...
func (c *client) buildRequest(req provider.Request) chatRequest {
	...
	case c.minimax:
		...
		out.ReasoningEffort = ""
	case c.mimo:
		// MiMo uses reasoning_effort (low/medium/high) for thinking depth control.
		// The thinking field is not required as MiMo uses reasoning_effort directly.
		// reasoning_content in the response carries the chain-of-thought.
	}
	return out
}
