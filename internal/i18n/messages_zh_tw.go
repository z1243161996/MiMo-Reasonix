...
var ChineseTraditional = Messages{
	...
	ListHooksNone:       "無生效 hooks — 在 .mimo-reasonix/settings.json（專案，需信任後）或 ~/.mimo-reasonix/settings.json（全域）設定",
	ListMcpHeader:       "MCP 伺服器",
	ListMcpNone:         "未連線 MCP 伺服器 — 在 mimo-reasonix.toml（[[plugins]]）或專案 .mcp.json 中新增",
...
設定：
  ...
  金鑰透過 api_key_env 從環境變數注入（如 MIMO_API_KEY）。
  執行 'mimo-reasonix setup' 生成設定；詳見 docs/SPEC.md。
`,
