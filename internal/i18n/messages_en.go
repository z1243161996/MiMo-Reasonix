...
var English = Messages{
	...
	SetKeyHint:      "Set your API key, then run `mimo-reasonix`.",
	ConfigLabel:     "config",
	ModelsLabel:     "models",
	...
	StepScaffold:    "scaffold mimo-reasonix.toml",
	StepSetKey:      "set API key",

	InitHint:       "Project memory (AGENTS.md) is generated in-session: run `mimo-reasonix`, then `/init` — the model analyzes the codebase and writes it. For configuration, use `mimo-reasonix setup`.",
	StepSetKeyHint: "run `mimo-reasonix setup`, or export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "interactive session",
	StepRunDesc:    "one-shot task",
	HelpFooter:     "mimo-reasonix help · all commands",

	ChatTip:           "Context is kept across turns. Type 'exit' or Ctrl-D to quit.",
	TurnCancelled:     "cancelled — back to prompt",
	NoSessionToResume: "no saved session to resume — start a new one with `mimo-reasonix`",
	...
	OutputStyleHint:    "set agent.output_style in mimo-reasonix.toml to apply one (takes effect next session)",
	ThemeHeader:        "themes:",
	...
	SlashMCPNone:                 "no MCP servers configured — add a [[plugins]] entry in mimo-reasonix.toml",
	...
	ListHooksNone:       "hooks: none active — configure in .mimo-reasonix/settings.json (project, after trust) or ~/.mimo-reasonix/settings.json (global)",
	ListMcpHeader:       "mcp servers",
	ListMcpNone:         "mcp: no servers connected — add one in mimo-reasonix.toml ([[plugins]]) or a project .mcp.json",
	...
	MissingKeyIntro:       "mimo-reasonix.toml is ready — just an API key away.",
	WroteFileFmt:          "Wrote %s",
	SetupComplete:         "Setup complete.",
	SetupCancelled:        "setup cancelled.",
	TryHintFmt:            "Try: %s",
	NextHint:              "Next: set your API key (run `mimo-reasonix setup` or export DEEPSEEK_API_KEY=...), then run `mimo-reasonix run \"your task\"`.",
	...
	SkipStaleCustomEntryFmt:    "skipping stale %q entry from mimo-reasonix.toml (pointing at %s) — please remove it from [[providers]]",
	...
	UsageRunHint:              "usage: mimo-reasonix run [--model NAME] <task>",
	ErrorPrefix:               "error:",
	...
	ProviderErrAuth:                "Authentication failed (HTTP 401): your API key is missing or unset. Add it to .env or run `mimo-reasonix setup`.",
	ProviderErrAuthRejected:        "Authentication failed (HTTP 401): the server rejected your API key. It may be wrong or expired, or the provider hit a transient auth/quota issue — retried with backoff and still failed. Try again shortly, or check the key in .env / run `mimo-reasonix setup`.",
	...
	UsageBody: `mimo-reasonix — a config- and plugin-driven coding agent (multi-model)

Usage:
  mimo-reasonix [--model NAME] [-c|--continue] [--resume] [--yolo] [--dir PATH]   interactive session (multi-turn; -c resumes the latest, --resume picks one)
  mimo-reasonix run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] <task>   run one task and exit
  mimo-reasonix review [--base BRANCH] [--commit SHA] [--model NAME]  AI-powered code review on local diffs
  mimo-reasonix serve [--model NAME] [--addr HOST:PORT] [--auth none|token|password] [--token STR] [--password STR] [--hash-password]  serve over HTTP+SSE (with optional auth)
  mimo-reasonix acp [--model NAME]                           serve Agent Client Protocol over stdio (also: mimo-reasonix --acp)
  mimo-reasonix setup [path]                                 interactive config wizard; writes mimo-reasonix.toml (+ .env)
  mimo-reasonix config auto-plan [off|on]                    configure automatic plan mode
  mimo-reasonix config memory-v5 [off|on|status]             configure Memory v5
  mimo-reasonix config reasoning-language [auto|zh|en]        configure visible reasoning language
  mimo-reasonix mcp <add|remove|list|import>                 manage MCP servers in mimo-reasonix.toml
  mimo-reasonix init                                         show how to generate project memory (AGENTS.md)
  mimo-reasonix doctor [--json]                              print redacted local diagnostics
  mimo-reasonix bot start|doctor|weixin-login                multi-channel IM bot gateway
  mimo-reasonix upgrade [--check] [--force]                   self-update to the latest release (also: mimo-reasonix update)
...
Examples:
  ...
  mimo-reasonix run "implement the TODOs in main.go"
  mimo-reasonix run --model mimo-pro "add unit tests for this function"
  echo "explain this code" | mimo-reasonix run

Configuration:
  Resolution: flag > ./mimo-reasonix.toml > ~/.mimo-reasonix/config.toml > built-in defaults
  Secrets come from the environment via api_key_env (e.g. MIMO_API_KEY).
  Run 'mimo-reasonix setup' to scaffold a config; see docs/SPEC.md.
`,
}
