...
func (t *installSkillTool) Description() string {
	scope := "'global' (only option — no project workspace) writes to the MiMo-Reasonix home skills directory."
	if t.store.HasProjectScope() {
		scope = "'project' (default) writes to <repo>/.mimo-reasonix/skills/ (this workspace only); 'global' writes to the MiMo-Reasonix home skills directory (every project)."
	}
