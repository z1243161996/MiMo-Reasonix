...
import (
	...
	"mimo-reasonix/internal/diff"
	"mimo-reasonix/internal/event"
	"mimo-reasonix/internal/evidence"
	"mimo-reasonix/internal/instruction"
	"mimo-reasonix/internal/jobs"
	"mimo-reasonix/internal/memory"
	"mimo-reasonix/internal/memorycompiler"
	"mimo-reasonix/internal/nilutil"
	"mimo-reasonix/internal/planmode"
	"mimo-reasonix/internal/provider"
	"mimo-reasonix/internal/repair"
...
type Agent struct {
	...
	// repairPipeline handles tool-call pathologies: truncated arguments,
	// embedded tool calls in reasoning text, and death-spiral loop detection.
	repairPipeline *repair.Pipeline

	// ctxMgr is the optional context manager providing token counting,
	// prefix caching, and fold decision support. When nil, the agent
	// relies solely on the built-in compaction logic in compact.go.
	ctxMgr ctxManagerField
}
...
func New(prov provider.Provider, tools *tool.Registry, session *Session, opts Options, sink event.Sink) *Agent {
	...
	a.repairPipeline = repair.NewPipeline()
	return a
}
...
func (a *Agent) Run(ctx context.Context, input string) (runErr error) {
	...
	for step := 0; a.maxSteps <= 0 || step < a.maxSteps || graceRound; step++ {
		...
		// Scavenge: if the model didn't emit proper tool calls but the text
		// contains embedded tool calls (e.g. inside markdown code blocks or
		// angle-bracket tags), extract them. This handles the common case where
		// the model outputs tool calls as text instead of using the proper
		// tool-call mechanism.
		if len(calls) == 0 && a.repairPipeline != nil {
			if scavenged := a.repairPipeline.ScavengeCalls(text); len(scavenged) > 0 {
				for _, sc := range scavenged {
					calls = append(calls, provider.ToolCall{
						ID:        fmt.Sprintf("scavenged-%d", len(calls)),
						Name:      sc.Name,
						Arguments: sc.Arguments,
					})
				}
				a.sink.Emit(event.Event{Kind: event.Notice, Level: event.LevelInfo,
					Text: fmt.Sprintf("scavenged %d tool call(s) from text", len(scavenged))})
			}
		}
...
func (a *Agent) executeOne(ctx context.Context, call provider.ToolCall) toolOutcome {
	...
	// Repair truncated tool-call arguments before execution. This handles the
	// common case where the model's output tokens are exhausted mid-argument,
	// leaving incomplete JSON that would otherwise fail with a parse error.
	args := call.Arguments
	if a.repairPipeline != nil {
		if repaired, fixed := a.repairPipeline.RepairArgs(args); fixed {
			a.sink.Emit(event.Event{Kind: event.Notice, Level: event.LevelInfo,
				Text: fmt.Sprintf("repaired truncated arguments for %s", call.Name)})
			args = repaired
		}
	}
	...
	// Use the repaired arguments for execution.
	result, err := t.Execute(cctx, json.RawMessage(args))
	if a.evidence != nil {
		if call.Name == "complete_step" {
			rec := evidence.ReceiptFromToolCall(call.Name, json.RawMessage(args), err == nil, t.ReadOnly())
	...
	if a.hooks != nil {
		a.hooks.PostToolUse(ctx, call.Name, json.RawMessage(args), result)
	}
	if err != nil {
		detail := result
