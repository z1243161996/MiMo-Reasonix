...
func TestExtractFromTarGz(t *testing.T) {
	// Build a .tar.gz in memory containing a "mimo-reasonix" entry.
	...
	got, err := extractFromTarGz(buf.Bytes(), "mimo-reasonix")
	if err != nil {
...
func TestExtractFromTarGz_Nested(t *testing.T) {
	...
	if err := tw.WriteHeader(&tar.Header{
		Name: "mimo-reasonix-linux-amd64/mimo-reasonix",
		Mode: 0o755,
		Size: int64(len(body)),
	}); err != nil {
		t.Fatal(err)
...
func TestExtractFromTarGz_NotFound(t *testing.T) {
	...
	_, err := extractFromTarGz(buf.Bytes(), "mimo-reasonix")
	if err == nil {
