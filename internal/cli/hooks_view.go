...
func renderHooks(width int, hooks []hook.ResolvedHook, trusted bool, projectDefines bool) string {
	...
	case trusted:
		b.WriteString(viewHint(viewCompactText("project trusted · config: project .mimo-reasonix/settings.json + global ~/.mimo-reasonix/settings.json", viewBudget(width, 2))))
	default:
		b.WriteString(viewHint(viewCompactText("project not trusted · config: project .mimo-reasonix/settings.json + global ~/.mimo-reasonix/settings.json", viewBudget(width, 2))))
	}
