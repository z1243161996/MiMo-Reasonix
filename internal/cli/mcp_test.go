...
func TestMCPEditConfigLaunchEditorRejectsUnterminatedQuote(t *testing.T) {
	...
	_, err := mcpEditConfigLaunchCommand("/tmp/mimo-reasonix.toml", func(string) (string, error) {
		t.Fatal("lookPath should not be called when VISUAL is set")
		return "", errors.New("unexpected lookup")
	})
	if err == nil {
...
func TestMCPEditConfigLaunchEditorExpandsTilde(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			t.Setenv("EDITOR", "")
			launch, err := mcpEditConfigLaunchCommand("/tmp/mimo-reasonix.toml", func(string) (string, error) {
				t.Fatal("lookPath should not be called when VISUAL is set")
				return "", errors.New("unexpected lookup")
			})
			if err != nil {
...
func TestMCPEditConfigLaunchFallsBackToTerminalEditor(t *testing.T) {
	...
	launch, err := mcpEditConfigLaunchCommand("/tmp/mimo-reasonix.toml", func(name string) (string, error) {
		if name == "vim" {
	...
	if len(launch.cmd.Args) != 2 || launch.cmd.Args[0] != "/usr/bin/vim" || launch.cmd.Args[1] != "/tmp/mimo-reasonix.toml" {
...
func TestApplyMCPModeDropsLegacyTier(t *testing.T) {
	...
	if err := cfg.SaveTo("mimo-reasonix.toml"); err != nil {
	...
	m.mcp = &mcpManager{
		stage: mcpStageMode,
		name:  "github",
		snapshot: mcpSnapshot{configPath: "mimo-reasonix.toml", servers: []mcpServerView{{
	...
	raw, err := os.ReadFile("mimo-reasonix.toml")
	if err != nil {
