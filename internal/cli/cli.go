// Package cli implements MiMo-Reasonix's command-line entry: subcommand routing, flag
...
func Run(args []string, version string) int {
	...
	case "init":
		...
		// for config), so `mimo-reasonix init` isn't a dead end.
		configureCLIThemeFromConfigNoProbe()
		return initHint()
	case "acp":
		configureCLIThemeFromConfigNoProbe()
	...
	case "version", "--version", "-v":
		fmt.Println("mimo-reasonix", version)
		return 0
	case "help", "--help", "-h":
		usage()
		return 0
...
func runServe(args []string) int {
	...
	*model = modelForResumePath(*model, *resume, cfg)
	...
	fmt.Printf("mimo-reasonix serve — %s on http://%s\n", ctrl.Label(), *addr)
...
func chatREPL(args []string) int {
	fs := flag.NewFlagSet("mimo-reasonix", flag.ContinueOnError)
	model := fs.String("model", "", "provider name (default: config default_model)")
	maxSteps := fs.Int("max-steps", 0, "max tool-call rounds (0 = use config/default)")
	cont := fs.Bool("continue", false, "resume the most recent saved session")
	fs.BoolVar(cont, "c", false, "shorthand for --continue")
...
// store. Keys always go to MiMo-Reasonix's global .env so they
...
// project-local mimo-reasonix.toml only when the user config dir can't be resolved.
func defaultConfigTarget() string {
	if p := config.UserConfigPath(); p != "" {
		return p
	}
	return "mimo-reasonix.toml"
}

// defaultEnvTarget is the display target for the mimo-reasonix-owned global
// MiMo-Reasonix global .env.
func defaultEnvTarget() string {
	return config.CredentialsTargetDescription()
}

// resolveSetupTargets picks where `mimo-reasonix setup` writes. Keys always go to the
// global env. The config goes to the user-global dir by default, to ./mimo-reasonix.toml
...
// setupConfig runs the configuration wizard (the `mimo-reasonix setup` command),
// writing config.toml to the user-global dir (or ./mimo-reasonix.toml under --local)
// and API keys to MiMo-Reasonix's global .env — never a project's own .env.
...
// initHint handles `mimo-reasonix init`. Unlike a config scaffold, project memory is
...
// (and to `mimo-reasonix setup` for config) so the verb isn't a dead end.
func initHint() int {
...
// and any entered API keys to MiMo-Reasonix's global .env. The wizard
...
func interactiveSetup(configPath, envPath string) int {
	...
	fmt.Print(boxed([]string{
		accent("◆") + " " + fmt.Sprintf(i18n.M.WelcomeTitleFmt, bold("mimo-reasonix")),
		"",
...
// for MiMo-Reasonix's global .env later and skips re-asking. A blank entry is fine —
...
// that older versions of the wizard wrote into mimo-reasonix.toml. They collide
...
// can warn the user to clean up mimo-reasonix.toml by hand.
...
// duplicate provider entries on subsequent re-runs of `mimo-reasonix setup`.
// The host-based slug also gives users a meaningful name to grep for in
// mimo-reasonix.toml. Falls back to a short sha1 of the raw URL when the URL
...
// not exist in config because users editing mimo-reasonix.toml deal with SKU names
// directly.
...
func familyOf(name string) providerFamily {
	switch {
	case strings.HasPrefix(name, "mimo"), strings.HasPrefix(name, "xiaomi"):
		return providerFamily{key: "mimo", name: "MiMo", desc: "fast & cheap, plus a stronger Pro SKU"}
...
// — the key step happens later in the wizard and MiMo-Reasonix's global .env is updated then.
...
// MiMo-Reasonix global .env. Re-pinning keeps hand-edited or previously saved values
...
// are appended — so re-running `mimo-reasonix setup` with a corrected key replaces the
...
func configReasoningLanguageCommand(args []string) int {
	...
	local := fs.Bool("local", false, "write ./mimo-reasonix.toml instead of the user config")
	if err := fs.Parse(args); err != nil {
		return 2
	}
	rest := fs.Args()
...
func configUsage() {
	fmt.Print(`Usage:
  mimo-reasonix config auto-plan [off|on]
  mimo-reasonix config memory-v5 [off|on|status]
  mimo-reasonix config reasoning-language [--local] [auto|zh|en]
`)
}
