...
func TestMCPMigrationWaitsForCLIWorkspace(t *testing.T) {
	isolateCLIConfigHome(t)
	cwd := mustGetwd(t)
	if err := os.WriteFile(filepath.Join(cwd, "mimo-reasonix.toml"), []byte(`
[[plugins]]
name = "cwd-project"
...
func TestRunMigratesLegacyConfigBeforeConfigOnlyCommands(t *testing.T) {
	t.Skip("legacy reasonix.toml migration path no longer applies after branding change")
	isolateCLIConfigHome(t)
...
func TestConfigAutoPlanIgnoresProjectConfig(t *testing.T) {
	...
	if err := os.WriteFile("mimo-reasonix.toml", []byte("[agent]\nauto_plan = \"on\"\n"), 0o644); err != nil {
...
func TestConfigReasoningLanguageLocalCreatesMinimalProjectOverride(t *testing.T) {
	...
	body, err := os.ReadFile("mimo-reasonix.toml")
	if err != nil {
...
func TestProvidersWithMissingKeysSkipsDisabledAutoPlanClassifier(t *testing.T) {
	cfg := config.Default()
	cfg.Providers = append(cfg.Providers, config.ProviderEntry{Name: "deepseek-flash", Kind: "openai", BaseURL: "https://api.deepseek.com", Model: "deepseek-v4-flash", APIKeyEnv: "DEEPSEEK_API_KEY"})
	cfg.Agent.AutoPlan = "off"
	cfg.Agent.AutoPlanClassifier = "deepseek-flash/deepseek-v4-flash"
	t.Setenv("DEEPSEEK_API_KEY", "")
	t.Setenv("MIMO_API_KEY", "test-key")
...
func TestConfigureKeys(t *testing.T) {
	// Force a clean baseline: any MIMO_API_KEY in the
	...
	if len(env) != 1 {
		t.Fatalf("env = %v (want 1: MiMo asked once)", env)
	}
	if env[0] != "MIMO_API_KEY=mimo-key" {
...
func TestConfigureKeysReusesExistingEnv(t *testing.T) {
	t.Setenv("MIMO_API_KEY", "preset-mimo-key")
	...
	if len(env) != 1 {
		t.Fatalf("env = %v (want 1: MiMo reused)", env)
	}
	if env[0] != "MIMO_API_KEY=preset-mimo-key" {
		t.Errorf("env[0] = %q, want re-pinned existing value", env[0])
	}
	if !strings.Contains(output.String(), "MIMO_API_KEY") {
		t.Errorf("expected a 'reusing' confirmation for MIMO_API_KEY, got:\n%s", output.String())
	}
}

func TestConfigureKeysCanResetExistingEnv(t *testing.T) {
	t.Setenv("MIMO_API_KEY", "stale-mimo-key")
	...
	if len(env) != 1 {
		t.Fatalf("env = %v (want 1: MiMo reset)", env)
	}
	if env[0] != "MIMO_API_KEY=fresh-mimo-key" {
		t.Errorf("env[0] = %q, want freshly entered value", env[0])
	}
	if !strings.Contains(output.String(), "[y/N]:") || !strings.Contains(output.String(), "MIMO_API_KEY") {
		t.Errorf("expected a reset confirmation for MIMO_API_KEY, got:\n%s", output.String())
	}
}
...
func TestConfigureKeysAllSetDefaultsToReusingInput(t *testing.T) {
	...
	env := configureKeys(selected, strings.NewReader("\n"), io.Discard)
	if len(env) != 1 {
		t.Errorf("env = %v, want 1 (MiMo reused)", env)
	}
}
...
// "mimo" and "deepseek" families, preserving the order each family first appears in.
func TestGroupByFamily(t *testing.T) {
	order, members, info := groupByFamily(config.Default().Providers)

	if got := order; !reflect.DeepEqual(got, []string{"mimo", "deepseek"}) {
		t.Fatalf("family order = %v, want [mimo deepseek]", got)
	}
	if got := members["mimo"]; !reflect.DeepEqual(got, []int{0, 1}) {
		t.Errorf("mimo members = %v, want [0 1]", got)
	}
	if got := members["deepseek"]; !reflect.DeepEqual(got, []int{2, 3}) {
		t.Errorf("deepseek members = %v, want [2 3]", got)
	}
	if info["mimo"].name != "MiMo" {
		t.Errorf("display name = %q", info["mimo"].name)
	}
...
func TestWriteDefaultConfigOmitsLegacyInternalMCPSections(t *testing.T) {
	path := filepath.Join(t.TempDir(), "mimo-reasonix.toml")
