...
func TestACPInitializesWithoutAPIKey(t *testing.T) {
	...
	if !strings.Contains(out, `"protocolVersion":1`) || !strings.Contains(out, `"name":"mimo-reasonix"`) {
...
func TestACPFactoryLoadsSessionCwdProjectConfig(t *testing.T) {
	...
	project := t.TempDir()
	if err := os.WriteFile(filepath.Join(project, "mimo-reasonix.toml"), []byte(`
default_model = "local"
...
`), 0o644); err != nil {
	...
	cmdDir := filepath.Join(project, ".mimo-reasonix", "commands")
