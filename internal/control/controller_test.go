...
import (
	...
	"mimo-reasonix/internal/checkpoint"
...
func TestRemoveMCPServerRemovesUnconnectedLazyPlaceholder(t *testing.T) {
	...
	t.Chdir(dir)
	if err := os.WriteFile("mimo-reasonix.toml", []byte(`
[[plugins]]
name = "mock"
command = "mock-mcp"
tier = "lazy"
...
func TestReloadCommandsFromFilesystem(t *testing.T) {
	...
	wsRoot := t.TempDir()
	cmdDir := filepath.Join(wsRoot, ".mimo-reasonix", "commands")
...
// (higher priority) wins. ConventionDirs = [".mimo-reasonix", ".agents", ".agent",
// ".claude"], scanned in reverse, so .mimo-reasonix is highest priority.
func TestReloadCommandsSameNameAcrossDirs(t *testing.T) {
	...
	// Higher priority: .mimo-reasonix/commands
	reasonixDir := filepath.Join(wsRoot, ".mimo-reasonix", "commands")
	...
	// The winning version should be from .mimo-reasonix (highest priority)
	sent, ok := c.CustomCommand("/greet world")
	if !ok {
		t.Fatal("/greet should be found")
	}
	if !strings.Contains(sent, "Hello from Reasonix") {
		t.Errorf("expected .mimo-reasonix version to win, got render: %q", sent)
	}
}
