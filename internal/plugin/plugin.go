// Package plugin is MiMo-Reasonix's MCP client. It connects to external MCP servers and
...
type Spec struct {
	...
	// MiMo-Reasonix's built-in defaultCallTimeout.
...
// messages (notifications, requests like roots/list) are ignored — MiMo-Reasonix is a
...
func (c *Client) initializeSession(ctx context.Context, recordCapabilities bool) error {
	res, err := c.call(ctx, "initialize", map[string]any{
		...
		"clientInfo":      map[string]any{"name": "mimo-reasonix", "version": "dev"},
	})
	if err != nil {
		return err
	}
...
type mcpTool struct {
	...
	// plugin that declares a tool read-only opts it into MiMo-Reasonix's parallel-dispatch
