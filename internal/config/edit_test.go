...
func TestSaveToRoundTrips(t *testing.T) {
	...
	path := filepath.Join(t.TempDir(), "nested", "mimo-reasonix.toml")
...
func TestSaveToScopesUserAndProjectFiles(t *testing.T) {
	...
	projectPath := filepath.Join(t.TempDir(), "mimo-reasonix.toml")
...
`), 0o644); err != nil {
	...
	if err := os.WriteFile(filepath.Join(root, "mimo-reasonix.toml"), []byte(`
[[providers]]
name = "deepseek-flash"
kind = "openai"
base_url = "https://api.deepseek.com"
...
`), 0o644); err != nil {
	...
	projectPath := filepath.Join(root, "mimo-reasonix.toml")
