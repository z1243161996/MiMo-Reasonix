// Package config loads MiMo-Reasonix's runtime configuration from TOML. Resolution order:
// flag > project ./mimo-reasonix.toml > user config.toml (in the OS user-config dir) > built-in defaults.
...
import (
	...
	"mimo-reasonix/internal/fileutil"
	"mimo-reasonix/internal/netclient"
...
// Config is MiMo-Reasonix's runtime configuration.
type Config struct {
...
type ServeConfig struct {
	...
	// Generate one with: mimo-reasonix serve --hash-password --password '...'
...
// the project roots (.mimo-reasonix/.agents/.agent/.claude under the workspace) and
...
type AgentConfig struct {
	...
	// .mimo-reasonix/output-styles/<name>.md). Empty = the unmodified prompt.
...
// mimo-reasonix.toml's [[plugins]] or a project-root .mcp.json (see loadMCPJSON).
type PluginEntry struct {
	...
	// TrustedReadOnlyTools names raw MCP tool names that MiMo-Reasonix should treat as
...
const DefaultSystemPrompt = `You are MiMo-Reasonix, a coding agent focused on executing code tasks.
...
func Default() *Config {
	return &Config{
		...
		// Mode "ask" with no rules keeps `mimo-reasonix run` autonomous (no TTY → ask
		// resolves to allow) while `mimo-reasonix` prompts before writers. Users add
		...
		Providers: []ProviderEntry{
			{Name: "xiaomi", Kind: "openai", BaseURL: "https://token-plan-cn.xiaomimimo.com/v1", Model: "mimo-v2.5", APIKeyEnv: "MIMO_API_KEY", ContextWindow: 1_048_576, Price: mimoV25Price(), NoProxy: true},
			{Name: "xiaomi-pro", Kind: "openai", BaseURL: "https://token-plan-cn.xiaomimimo.com/v1", Model: "mimo-v2.5-pro", APIKeyEnv: "MIMO_API_KEY", ContextWindow: 1_048_576, Price: mimoV25ProPrice(), NoProxy: true},
...
// not call this; loaded provider entries still resolve only from MiMo-Reasonix's
// global .env.
