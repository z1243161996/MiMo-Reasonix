...
func reasonixHomeDir() string {
	...
	if runtimeGOOS == "windows" {
		if dir := osUserConfigDir(); dir != "" {
			return filepath.Join(dir, "mimo-reasonix")
		}
		if home, err := osUserHomeDir(); err == nil && home != "" {
			return filepath.Join(home, "AppData", "Roaming", "mimo-reasonix")
		}
		return ""
	}
	if home, err := osUserHomeDir(); err == nil && home != "" {
		return filepath.Join(home, ".mimo-reasonix")
	}
	if dir := osUserConfigDir(); dir != "" {
		return filepath.Join(dir, "mimo-reasonix")
	}
...
func legacyXDGConfigPaths() []string {
	...
	if dir := cleanEnvDir("XDG_CONFIG_HOME"); dir != "" {
		add(filepath.Join(dir, "mimo-reasonix", "config.toml"))
	}
	if home, err := osUserHomeDir(); err == nil && home != "" {
		add(filepath.Join(home, ".config", "mimo-reasonix", "config.toml"))
	}
	return paths
}
...
func legacyOSSupportDir() string {
	...
	path := filepath.Join(dir, "mimo-reasonix")
...
// real location instead of a hardcoded ~/.mimo-reasonix path.
func userConfigDisplayPath() string {
	p := userConfigPath()
	if p == "" {
		return "<os-config-dir>/mimo-reasonix/config.toml"
	}
	if home, err := osUserHomeDir(); err == nil && home != "" {
...
// UserConfigPath is the user-global config.toml. It lives under MiMo-Reasonix home:
// REASONIX_HOME/config.toml, then ~/.mimo-reasonix/config.toml on Unix-like systems,
// or %AppData%/mimo-reasonix/config.toml on Windows. If %AppData% is unavailable on
// Windows, it falls back to %USERPROFILE%/AppData/Roaming/mimo-reasonix/config.toml.
...
// ReasonixHomeDir is the current MiMo-Reasonix home directory. It honors
// REASONIX_HOME, then uses ~/.mimo-reasonix on macOS/Linux or %APPDATA%/mimo-reasonix on
// Windows, with a %USERPROFILE%/AppData/Roaming fallback when %APPDATA% is
// unavailable.
func ReasonixHomeDir() string { return reasonixHomeDir() }

// UserCredentialsPath is the mimo-reasonix-owned global .env file under MiMo-Reasonix
// home. It is the single source for provider credentials saved by MiMo-Reasonix, so
// stale shell, Windows, project, or home env vars cannot silently override keys
// the user saved through setup or settings. "" when MiMo-Reasonix home can't be
// resolved.
...
// commands), in canonical-first order. .mimo-reasonix is ours; .agents / .agent /
...
// .mimo-reasonix/settings.json (see internal/hook).
var ConventionDirs = []string{".mimo-reasonix", ".agents", ".agent", ".claude"}

// conventionSubdirsAsc joins sub under each ConventionDir of base, in ascending
// priority (reverse of ConventionDirs) so the canonical .mimo-reasonix ends up the
...
// ~/.mimo-reasonix/commands), the MiMo-Reasonix home commands dir, the legacy OS
// app-support dir if different, then the project's
// convention dirs (.claude/commands … .mimo-reasonix/commands). Scanning the .claude /
...
func SourcePathForRoot(root string) string {
	root = resolveRoot(root)
	projectTOML := "mimo-reasonix.toml"
	if root != "." {
		projectTOML = filepath.Join(root, "mimo-reasonix.toml")
	}
