...
const (
	...
	credentialsKeyringService = "mimo-reasonix"
...
func loadCredentialStoreForRoot(root string) {
	...
	if p := UserCredentialsPath(); p != "" {
		loadDotEnvFileAs(p, CredentialSource{Kind: CredentialSourceCredentials, Path: p, Label: "MiMo-Reasonix credentials (.env)"})
	}
}
...
func pinCredentialAssignments(assignments map[string]string) {
	for key, value := range assignments {
		...
		recordCredentialSource(key, value, CredentialSource{Kind: CredentialSourceCredentials, Path: UserCredentialsPath(), Label: "MiMo-Reasonix credentials (.env)"})
	}
}
...
func credentialSourceLabel(source CredentialSource) string {
	...
	case CredentialSourceCredentials:
		return "MiMo-Reasonix credentials"
	case CredentialSourceHomeEnv:
		return "home .env"
	case CredentialSourceLegacy:
		return "legacy MiMo-Reasonix credentials"
