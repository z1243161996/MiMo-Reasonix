...
func TestLoadForEditMigratesLegacyMCPTiers(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "mimo-reasonix.toml")
	body := `
[[plugins]]
name = "playwright"
command = "npx"
...
`
	...
	// The config is normalized on load, so additional default providers may be added.
	// Just verify the original plugin and provider are preserved.
	if !strings.Contains(string(updated), `command = "npx"`) {
		t.Fatalf("plugin command should be preserved:\n%s", updated)
	}
}
