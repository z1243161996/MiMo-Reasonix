...
func TestTrustPluginReadOnlyToolInSourceForRootUpdatesMCPJSON(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(root, "mimo-reasonix.toml")); !os.IsNotExist(err) {
...
command = "local-bin"
`
	if err := os.WriteFile("mimo-reasonix.toml", []byte(toml), 0o644); err != nil {
		t.Fatal(err)
	}
	mcp := `{ "mcpServers": {
  ...
  "extra":  { "command": "extra-bin", "auto_start": false }
	...
	if byName["shared"].Command != "local-bin" || byName["shared"].URL != "" {
		t.Errorf("mimo-reasonix.toml should win the collision, got %+v", byName["shared"])
	}
...
func TestLoadMergesPluginsAcrossTOMLSources(t *testing.T) {
	...
	if err := os.WriteFile("mimo-reasonix.toml", []byte("[[plugins]]\nname = \"projectmcp\"\ncommand = \"project-bin\"\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if !names["globalmcp"] || !names["projectmcp"] {
		t.Fatalf("a project mimo-reasonix.toml [[plugins]] dropped the global config's server; got %+v", cfg.Plugins)
	}
}
...
func TestMergeMCPJSONPrecedence(t *testing.T) {
	// mimo-reasonix.toml already declares "shared" (stdio); .mcp.json offers a colliding
	// "shared" (http) plus a fresh "extra". mimo-reasonix.toml must win on the collision;
	// "extra" gets appended.
	...
	if cfg.Plugins[0].Name != "shared" || cfg.Plugins[0].Command != "local-bin" || cfg.Plugins[0].URL != "" {
		t.Errorf("collision not won by mimo-reasonix.toml: %+v", cfg.Plugins[0])
	}
...
`), 0o644); err != nil {
  ...
  "dida": {
	...
	if source != "mimo-reasonix.toml" {
		t.Fatalf("source = %q, want mimo-reasonix.toml", source)
	}
	if updated.URL != "https://reasonix.example/mcp" {
		t.Fatalf("updated URL = %q", updated.URL)
	}

	projectRaw, err := os.ReadFile("mimo-reasonix.toml")
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(string(projectRaw), "access_token=toml") || strings.Contains(string(projectRaw), "Authorization") {
		t.Fatalf("mimo-reasonix.toml auth material should be removed:\n%s", projectRaw)
	}
