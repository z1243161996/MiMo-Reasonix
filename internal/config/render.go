...
func RenderTOMLForScope(c *Config, scope RenderScope) string {
	...
	b.WriteString("# MiMo-Reasonix configuration.\n")
	fmt.Fprintf(&b, "# Resolution order: flag > ./mimo-reasonix.toml > %s > built-in defaults.\n", userConfigDisplayPath())
	b.WriteString("# Fields marked user/global only are not overridden by ./mimo-reasonix.toml.\n")
	b.WriteString("# Secrets are named via api_key_env and stored in MiMo-Reasonix's global .env; never put keys here.\n\n")
	...
	if scope != RenderScopeProject {
		fmt.Fprintf(&b, "credentials_store = %q   # legacy compatibility; provider keys are saved in MiMo-Reasonix's global .env\n", normalizeCredentialsStore(c.CredentialsStore))
	}
	b.WriteString("\n")
