...
import (
	...
	"mimo-reasonix/internal/mcpdiag"
...
func pluginTOMLSourcePathForRoot(root, name string) string {
	...
	if resolved := resolveRoot(root); resolved != "." {
		projectTOML = filepath.Join(resolved, "mimo-reasonix.toml")
	}
...
func SaveMinimalProjectReasoningLanguage(path, lang string) (string, error) {
	...
	body := fmt.Sprintf(`# MiMo-Reasonix project configuration.
...
// user's global config when root has no mimo-reasonix.toml. Existing project files
