...
func migrateMCPToUserConfig(projectRoots []string) (*MCPGlobalMigrationResult, error) {
	...
	for _, root := range normalizedMCPMigrationRoots(projectRoots) {
		addEntries(loadPluginEntriesFromTOML(filepath.Join(root, "mimo-reasonix.toml")))
...
func legacyTOMLPaths(dest, home string) []string {
	...
	for _, legacy := range legacyXDGConfigPaths() {
		add(legacy)
		add(filepath.Join(filepath.Dir(legacy), "mimo-reasonix.toml"))
	}
	add(filepath.Join(filepath.Dir(dest), "mimo-reasonix.toml"))
	if home != "" {
		add(filepath.Join(home, ".reasonix", "mimo-reasonix.toml"))
	}
	return paths
}
