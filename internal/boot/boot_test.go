...
func TestBuildFoldsProjectMemoryIntoSystemPrompt(t *testing.T) {
	...
	writeFile(t, dir, "mimo-reasonix.toml", `
...
api_key_env = "REASONIX_TEST_KEY_UNSET"
`)
	writeFile(t, dir, ".mimo-reasonix/skills/projskill.md", "---\ndescription: a project skill\n---\nplaybook")
...
model = "x"
`)
	writeFile(t, dir, ".mimo-reasonix/skills/readonlydig/SKILL.md", `---
...
func TestBuildTokenEconomyPlanModeCanConnectAllowedMCPSource(t *testing.T) {
	...
	setBootTokenProfileTestProvider(t, prov)
	writeFile(t, dir, "mimo-reasonix.toml", fmt.Sprintf(`
default_model = "test-model"
...
func TestBuildOmitsExcludedSkillRootsFromPromptAndRuntimeList(t *testing.T) {
	...
	writeFile(t, home, ".mimo-reasonix/skills/keep.md", "---\ndescription: keep\n---\nplaybook")
...
func TestRememberPermissionRuleUsesWorkspaceRoot(t *testing.T) {
	...
	t.Chdir(cwd)
	writeFile(t, cwd, "mimo-reasonix.toml", `
[permissions]
allow = ["Bash(cwd*)"]
`)
	writeFile(t, workspace, "mimo-reasonix.toml", `
[permissions]
...
`)
	...
	cwdCfg := config.LoadForEdit(filepath.Join(cwd, "mimo-reasonix.toml"))
	...
	workspaceCfg := config.LoadForEdit(filepath.Join(workspace, "mimo-reasonix.toml"))
...
`)
	...
	if !res.Saved || res.Path != filepath.Join(workspace, "mimo-reasonix.toml") {
...
`)
	...
	if _, err := os.Stat(filepath.Join(cwd, "mimo-reasonix.toml")); !os.IsNotExist(err) {
...
`)
	...
	cfg := config.LoadForEdit(filepath.Join(workspace, "mimo-reasonix.toml"))
...
func TestBuildMigratesLegacyConfigEndToEnd(t *testing.T) {
	...
	// migrated plugins.
	writeFile(t, proj, "mimo-reasonix.toml", "")
...
func TestBuildMigratesLegacyXDGAndProjectSessions(t *testing.T) {
	...
	proj := robustTempDir(t)
	writeFile(t, proj, "mimo-reasonix.toml", "[codegraph]\nenabled = false\n")

	legacyRoot := filepath.Join(xdg, "mimo-reasonix")
...
`)
	...
	raw, err := os.ReadFile(filepath.Join(dir, "mimo-reasonix.toml"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestBuildDefaultsToNearestGitRoot(t *testing.T) {
	...
	writeFile(t, root, "mimo-reasonix.toml", `
