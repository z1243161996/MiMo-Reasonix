...
Starting with **MiMo-Reasonix v1.8.1**, MiMo-Reasonix uses one user-facing home directory
for global configuration and user-owned state. CLI and desktop share this
location.

## MiMo-Reasonix Home

| Platform | MiMo-Reasonix home |
| --- | --- |
| macOS | `~/.mimo-reasonix` |
| Linux | `~/.mimo-reasonix` |
| Windows | `%APPDATA%\mimo-reasonix` |

Set `MIMO_REASONIX_HOME` to override MiMo-Reasonix home for tests, CI, or portable
...
| Data | Path |
| --- | --- |
| Global config | `<MiMo-Reasonix home>/config.toml` |
| Global provider credentials | `<MiMo-Reasonix home>/.env` |
| Legacy credentials import source | `<MiMo-Reasonix home>/credentials` |
| Global slash commands | `<MiMo-Reasonix home>/commands/` |
| Global skills | `<MiMo-Reasonix home>/skills/` |
| Global hooks | `<MiMo-Reasonix home>/settings.json` |
| Hook trust store | `<MiMo-Reasonix home>/trust.json` |
| Sessions | `<MiMo-Reasonix home>/sessions/` |
| Archives | `<MiMo-Reasonix home>/archive/` |
| Memory | `<MiMo-Reasonix home>/memory/` and `<MiMo-Reasonix home>/projects/` |

The global user config is named `config.toml`. Project-local config files keep
the name `mimo-reasonix.toml`. If someone says "global mimo-reasonix.toml", they usually
mean `<MiMo-Reasonix home>/config.toml`.

## Global `config.toml`

`<MiMo-Reasonix home>/config.toml` stores non-secret configuration shared by the CLI
and desktop app. It may contain the same provider, plugin, UI, desktop, tool,
skill, sandbox, bot, and agent settings that MiMo-Reasonix renders into user config.
...
[[plugins]]
name    = "example"
command = "mimo-reasonix-plugin-example"
```
...
When a custom provider is added from the desktop settings or `mimo-reasonix setup`,
MiMo-Reasonix stores a generated `api_key_env` in `config.toml` and writes the secret
...
MiMo-Reasonix derives the default from the provider name. Names that normalize to
...
MiMo-Reasonix sends chat requests to `base_url + "/chat/completions"` and probes model
discovery candidates such as `/models` and `/v1/models`. If a gateway gives you a
complete chat request URL, set `chat_url`; MiMo-Reasonix will use it directly and will
not append `/chat/completions`. If model discovery needs a separate address, set
`models_url`.

## Global `.env`

`<MiMo-Reasonix home>/.env` is the single runtime source for provider API keys saved
by MiMo-Reasonix. The setup wizard, desktop settings, CLI missing-key prompts, and
...
GEMINI_API_KEY=...
ANTHROPIC_API_KEY=...
# mimo-reasonix-cleared OLD_API_KEY
```
...
- multiline values are rejected by MiMo-Reasonix writes;
- keys must use shell-style names such as `MIMO_API_KEY`;
- `# mimo-reasonix-cleared KEY` comments are non-secret tombstones written after a key
  is deleted so legacy stores do not silently re-import it;
- MiMo-Reasonix writes this file with restricted permissions where the OS supports
  them.

For provider requests, MiMo-Reasonix resolves only this global `.env`. Project `.env`
...
into the process environment, and MiMo-Reasonix control variables such as
`MIMO_REASONIX_HOME`, `MIMO_REASONIX_STATE_HOME`, and `XDG_CONFIG_HOME` are ignored there.

Caches remain in the OS cache directory, for example
`~/Library/Caches/mimo-reasonix` on macOS, `$XDG_CACHE_HOME/mimo-reasonix` or
`~/.cache/mimo-reasonix` on Linux, and `%LOCALAPPDATA%\mimo-reasonix\cache` on Windows.
Set `MIMO_REASONIX_CACHE_HOME` to override the cache root.
...
```text
command-line flags
> project ./mimo-reasonix.toml
> global <MiMo-Reasonix home>/config.toml
...
```text
macOS/Linux: ~/.mimo-reasonix/config.toml
Windows:     %APPDATA%\mimo-reasonix\config.toml
```

## Legacy Migration

Starting with **v1.8.1**, MiMo-Reasonix automatically checks legacy locations on
startup before the first config load. Migration is synchronous, one-time, and
non-destructive: old files are copied or converted to MiMo-Reasonix home and left
untouched.
...
Legacy credentials, memory files, and sessions are also imported into MiMo-Reasonix
home when the new destination does not already exist. Legacy provider keys are
copied into `<MiMo-Reasonix home>/.env` only when that file does not already contain
...
Starting in **v1.9.1**, MiMo-Reasonix also backfills MCP servers from known legacy
paths, legacy `config.json`, desktop-registered projects, and restored tab
projects into the global `<MiMo-Reasonix home>/config.toml`. Existing global
...
If MiMo-Reasonix has already created the new home directory but some legacy data was
...
MiMo-Reasonix checks the common layouts below that root and uses a source-specific
...
existing `<MiMo-Reasonix home>/config.toml`; if the new config already exists, copy
...
- `/migrate` is available only in Go-based MiMo-Reasonix builds that include the
  command. If MiMo-Reasonix reports `unknown command`, upgrade first and rerun it.
